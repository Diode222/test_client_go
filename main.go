...
func main() {
	...
	// FIXME diode test
	testClient := clientset.CoreV1().Pods(apiv1.NamespaceDefault)


	var r apiv1.ResourceRequirements
	j := `{"limits": {"cpu":"256m", "memory": "512Mi"}}`
	...
	podRes, err := testClient.Create(pod)
	if err != nil {
		fmt.Println("pod created failed.")
		panic(err)
	}

	fmt.Println(fmt.Sprintf("created pod %s", podRes.ObjectMeta.GetName()))
